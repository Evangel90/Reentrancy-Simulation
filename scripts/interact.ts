...
async function main() {
    ...
    const deposit = await victimContract.deposit({value: ethers.parseEther('0.1')});
    await deposit.wait();
    getBalance();
    
    const attack = await attackerContract.attack({value: ethers.parseEther('0.01'), gasLimit: 5000000});
    await attack.wait();
    getBalance();
